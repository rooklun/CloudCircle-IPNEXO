<template>
  <div class="bg-[#0D1117] text-gray-300 font-sans min-h-screen flex">
    <!-- Sidebar -->
    <aside
      class="w-70 bg-[#161B22] border-r border-gray-800 flex-shrink-0 flex flex-col"
    >
      <div
        class="h-16 flex items-center justify-center text-2xl font-bold text-white border-b border-gray-800"
      >
        <Logo class="mr-2" />
        <span>IPNEXO</span>
      </div>
      <nav class="flex-1 px-4 py-6 space-y-2">
        ...
        >
          <component :is="item.icon" class="h-5 w-5 mr-3 text-cyan-400" />
          {{ item.name }}
          <span
            v-if="item.tag"
        ...
        <div class="pt-6">
          ...
          <div class="mt-2 space-y-2">
            ...
            >
              <component
                :is="item.icon"
                class="h-5 w-5 mr-3 text-gray-500 group-hover:text-cyan-400"
              />
              {{ item.name }}
            </a>
          </div>
    ...
    <div class="flex-1 flex flex-col">
      <!-- Header -->
      <header
        class="h-16 bg-[#161B22]/80 backdrop-blur-sm border-b border-gray-800 flex items-center justify-between px-8"
      >
        ...
        <div class="flex items-center space-x-4">
          <button
            class="px-5 py-2 text-sm font-medium text-white bg-cyan-500 rounded-md hover:bg-cyan-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-cyan-500 transition-all duration-200"
          >
            充值
          </button>
          <div
      ...
      <main class="flex-1 p-8 overflow-y-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <button
              class="mt-6 w-full bg-cyan-500 text-white py-2.5 rounded-md hover:bg-cyan-600 font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-cyan-500"
            >
              立即购买
            </button>
          </div>
          ...
          >
            ...
            <p class="text-3xl font-bold text-gray-300">
              低至 <span class="text-cyan-400"><sup>¥</sup>48</span
              ><span class="text-base font-normal text-gray-500">/条/月</span>
            </p>
            <button
          ...
          >
            ...
            <p class="text-3xl font-bold text-gray-300">
              低至 <span class="text-cyan-400"><sup>¥</sup>20</span
              ><span class="text-base font-normal text-gray-500">/G</span>
            </p>
            <button
              class="mt-12 w-full bg-gray-700 text-gray-300 py-2.5 rounded-md hover:bg-gray-600 font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-gray-500"
            >
        ...
        <!-- Placeholder Content -->
        <div
          class="mt-16 flex flex-col items-center justify-center text-center text-gray-500"
        >
          <div class="relative w-48 h-48 mb-6">
            <div
              class="absolute inset-0 bg-cyan-400/10 rounded-full animate-ping"
            ></div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-48 w-48 text-gray-700"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <button
            class="mt-6 px-8 py-3 text-sm font-semibold text-white bg-cyan-500 rounded-md hover:bg-cyan-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-cyan-500 transition-all duration-200"
          >
            立即购买
          </button>
        </div>
    ...
    <div
      class="fixed right-4 bottom-1/2 translate-y-1/2 flex flex-col items-center space-y-3"
    >
      <button
        v-for="action in floatingActions"
        :key="action.name"
        class="w-12 h-12 bg-[#161B22] border border-gray-800 rounded-full flex items-center justify-center text-gray-400 hover:text-cyan-400 hover:border-cyan-500/50 transition-all duration-200 group relative"
      >
      ...
      <button
        class="w-12 h-12 bg-cyan-500 border border-cyan-500 rounded-full flex items-center justify-center text-white hover:bg-cyan-600 transition-all duration-200 group relative"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
